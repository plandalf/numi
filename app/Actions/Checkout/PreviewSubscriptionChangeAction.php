...
class PreviewSubscriptionChangeAction
{
    ...
    /**
     * Set the Stripe integration for testing purposes
     */
    public function setStripeIntegration(Stripe $integration): void
    {
    ...
    public function __invoke(CheckoutSession $session, ?string $effectiveAt = null): array
    {
        ...
        // Use injected integration for testing, otherwise get from session
        $integrationClient = $this->stripeIntegration ?? $session->integrationClient();
        ...
        foreach ($existingItems as $item) {
            ...
            if ($item->id === $existingBaseItem->id) {
                // Find the corresponding line item in checkout session
                $checkoutLineItem = $session->lineItems->first(function ($lineItem) use ($newBase) {
                    return $lineItem->price_id === $newBase['local_price_id'];
                });
                if ($checkoutLineItem) {
                    $newQuantity = $checkoutLineItem->quantity;
                }
            }
        ...
        return [
            ...
            'proposed' => [
                'base_item' => [
                    ...
                    'quantity' => $session->lineItems->first(function ($lineItem) use ($newBase) {
                        return $lineItem->price_id === $newBase['local_price_id'];
                    })?->quantity ?? 1,
                    'product' => [
    ...
    protected function resolveNewBaseRecurringLine(CheckoutSession $session): ?array
    {
        ...
        return [
            ...
            'amount' => $price->amount instanceof \Money\Money ? $price->amount->getAmount() : (int) $price->amount,
     ...
     */
    public function resolveExistingBaseItem(array $items, ?string $targetProductId)
    {
        if ($targetProductId) {
    ...
    public function resolveEffectiveTimestamp(?string $effectiveAt, ?int $trialEndTs, ?int $periodEndTs): array
    {
    ...
    public function tsOrNull($value): ?int
    {
        if (empty($value)) {
            return null;
        }
