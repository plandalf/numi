...
export default function IntegrationSetup({ app, integrationName: initialIntegrationName }: Props) {
  const { props } = usePage();
  ...
  useEffect(() => {
    if (props.csrf_token) {
      axios.defaults.headers.common['X-CSRF-TOKEN'] = props.csrf_token as string;
    }
  }, [props.csrf_token]);
  ...
  const handleOAuthFlow = async () => {
    console.log('Starting OAuth client credentials flow...', { app: app.key, name: integrationName, fields: integrationFields });
    ...
    try {
      ...
      console.log('OAuth integration created successfully:', response.data);
      ...
      if (response.data.success) {
        ...
        setStep('success');
        // Notify parent window that integration was created
        setTimeout(() => {
          if (window.opener) {
  ...
  const handleApiKeySetup = async () => {
    console.log('Starting integration creation...', { app: app.key, name: integrationName, fields: integrationFields });
  ...
  const handleBack = () => {
    if (step === 'choose') {
      // Close popup if we're on the choose step
      window.close();
    } else {
  ...
  useEffect(() => {
    ...
    if (urlParams.get('oauth_success') === 'true') {
      ...
      setTimeout(() => {
        if (window.opener) {
          window.opener.postMessage({ type: 'INTEGRATION_CREATED' }, '*');
          window.close();
        }
      }, 2000);
    } else if (urlParams.get('oauth_error')) {
      setError(urlParams.get('oauth_error') || 'OAuth authorization failed');
    }
  }, []);
  ...
  return (
    <>
      <Head title={`Setup ${app.name} Integration`} />
        ...
        <div className="w-full max-w-md">
          <Card className="relative">
            <CardHeader className="text-center">
              ...
              <CardTitle>Connect to {app.name}</CardTitle>
