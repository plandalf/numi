...
interface FulfillmentPageProps extends PageProps {
    order: Order;
}
...
const OrderItemCard = ({ item, orderUuid, errors, flash }: { 
    item: OrderItem; 
    orderUuid: string; 
    errors?: Record<string, string>;
    flash?: { success?: string; error?: string };
            ...
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h4 className="font-medium">{item.price.product.name}</h4>
                            <p className="text-sm text-gray-600">
                                Qty: {item.quantity} • ${item.total_amount} • {item.delivery_method.label}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Badge style={{ backgroundColor: item.fulfillment_status.color }}>
...
export default function Fulfillment({ order, errors, flash }: FulfillmentPageProps) {
    return (
        <AppLayout>
            <Head title={`Fulfillment - Order ${order.uuid}`} />
