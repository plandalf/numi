...
const buttonVariants = cva(
  "relative isolate inline-flex items-center justify-center gap-x-2 rounded-[var(--radius-lg)] before:rounded-[var(--radius-md)] after:rounded-[var(--radius-lg)] border text-base/6 font-semibold px-[calc(theme(spacing.3.5)-1px)] py-[calc(theme(spacing.2.5)-1px)] sm:px-[calc(theme(spacing.3)-1px)] sm:py-[calc(theme(spacing.1.5)-1px)] sm:text-sm/6 focus:not-data-focus:outline-hidden data-focus:outline-2 data-focus:outline-offset-2 data-focus:outline-blue-500 data-disabled:opacity-50 *:data-[slot=icon]:mx-0 *:data-[slot=icon]:my-0 *:data-[slot=icon]:size-5 *:data-[slot=icon]:shrink-0 *:data-[slot=icon]:self-center *:data-[slot=icon]:text-[--btn-icon] sm:*:data-[slot=icon]:my-0 sm:*:data-[slot=icon]:size-4 before:absolute before:inset-0 before:-z-10 before:shadow-smX dark:before:hidden dark:border-white/5 after:absolute after:inset-0 after:-z-10 after:shadXow-[inset_0_1px_theme(colors.white/15%)] data-active:after:bg-[--btn-hover-overlay] data-hover:after:bg-[--btn-hover-overlay] dark:after:-inset-px data-disabled:before:shadow-none data-disabled:after:shadow-none cursor-pointer outline-none transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive whitespace-nowrap",
  {
    variants: {
      variant: {
        default:
