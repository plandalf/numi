...
export const BorderPicker: React.FC<BorderPickerProps> = ({
  ...
  const [localRadius, setLocalRadius] = useState(typeof value.radius === 'string' ? value.radius : '');
  ...
  useEffect(() => {
    setLocalRadius(typeof value.radius === 'string' ? value.radius : '');
  }, [value.radius]);
  ...
  const handleRadiusChange = (newValue: number) => {
    ...
    onChange({ ...value, radius: formatted });
  };
    ...
    <div className={cn("py-4 px-2.5 flex flex-col items-center gap-2 w-full", className)}>
      ...
      <div className="flex flex-row gap-2 w-full items-center">
        ...
        <div className="flex flex-row gap-2 w-full">
          {/* <Select value={value.stroke} onValueChange={(newStroke) => onChange({ ...value, stroke: newStroke })}>
            <SelectTrigger title='Stroke' className="truncate cursor-pointer text-sm">
      ...
      <div className="flex flex-row gap-2 w-full items-center">
        ...
        <div className="flex flex-row gap-2 w-full">
          <RangeSlider
            ...
            max={40}
          >
            <div title='Border Radius' className="flex flex-row gap-2 bg-gray-200/50 rounded-md py-2 px-3 items-center w-full">
